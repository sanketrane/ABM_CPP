...
// model --
float sp_numbers(int Time, int t0, arma::vec params){ 
...
// Simulator function 
template <typename T>
std::vector<float> model_output (arma::vec params, std::vector<T> predictor_data){
  std::vector<float> sim_cal;
  // simuations
  for (int i=0; i<predictor_data.size(); i++){
    sim_cal.push_back(log(sp_numbers(predictor_data[i], predictor_data[0], params)));
  }
  return sim_cal;
}
...
template <typename T>
float llfunction(arma::vec params, T response_variable, T predictor_variable){
    int i, size_dat = response_variable.size();
    float  sd = params.at(3), ll_ind[size_dat], ll_est;
    std::vector<float> model_pred = model_output(params, predictor_variable);       // model simulation

    for (i=0; i<size_dat; i++){
      ll_ind[i] = normal_pdf(model_pred[i], response_variable[i], sd);
      ll_est += log(ll_ind[i]);
    }
    return ll_est;
}
...
int main (int argc, char * const argv[]) {
  ...
  arma::vec init_guess = {5.0, 0.2, 5.0, 1.5}, proposal_width = {0.5, 0.5, 0.5, 0.5}, prior_mu = {5, 0.1, 5, 1.2}, \
  prior_sd = {1, 0.5, 1, 0.5}, parms = {5, 0.05, 1.5};
  float sig_sp = parms[2], sp_noise[num_obs];
  ...
  std::vector<int> Time_obs = rand_sequence(tStart, tEnd, num_obs); std::sort(Time_obs.begin(), Time_obs.end());
  
  // generating ordered sequence
  std::vector<float> Time_pred(100); 
  float tStep= (tEnd - tStart)/100.0, startNum = tStart/1.0;
  std::generate(Time_pred.begin(), Time_pred.end(), [&startNum, &tStep]{ return startNum += tStep;});

  // generating data
  std::vector<float> sp_cal = model_output(parms, Time_obs) ;
  print_vector(sp_cal);

  // for (i=0; i<num_obs; i++){
  //   sp_noise[i] = randnorm<float>(0, sig_sp);
  //   std::cout << sp_noise[i] << '\n';
  // }
   std::vector<float> sp_dat;
   for (i=0; i<num_obs; i++){
    //std::cout << "sp num "  << sp_numbers(Time_obs[i], parms) << '\n';
    sp_dat.push_back(sp_cal[i] + randnorm<float>(0, sig_sp));
  }
   print_vector(sp_dat);
  ...
  //  (std::begin(sp_noise), std::end(sp_noise));
  //  for (int i: sp_dat) {
  //       std::cout << i << " ";
  //   }
  // print_vector(sp_dat);
