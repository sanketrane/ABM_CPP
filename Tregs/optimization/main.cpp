...
# define FILEPATH "/Users/sanketrane/Dropbox/Conference_Room_Files/Sanket/T_cell_ABM/new/"
# define PARAMETER_FILENAME "parameters_ASM8rhovar_Ki67gap_90.txt"

# define OUTPUT_FILENAME "allout_ASM8rhovar.csv"
...
# define LOG_GFP_MIN 0.f // lower bound on GFP detectability
# define LOG_GFP_MAX 4000.f // Max fluorescence for binning
# define LOG_GFPPOS_THRESHOLD 1600.f  // Taken from Thea's channel numbers data - by eye
# define LOG_GFP_MEAN_KHI 1800.f
# define LOG_GFP_SD_KHI 50.f
# define LOG_GFP_MEAN_KLO 2900.f // ln(2) higher than KHI
# define LOG_GFP_SD_KLO 50.f
# define logGFP_halfstep 300.f // decrement for GFP expression when cells divide. 300 is what I guessed from iSP and mSP
# define NUM_GFP_BINS 50 // for GFP histogram
# define GFP_LOSS_RATE 0.04f  // log(2)/4 .... 4d half life - we'll need to tweak/fit!
...
private:
    ...
    float logGFP; // Log of GFP expression. Max zero (GFP=1)
    float logKi67_det; // Log of Ki67 expression. Max zero (Ki67=1). Decays deterministically
    bool Ki67_stoch; // Ki67 expression hi/lo. Toggles on->off stochastically at rate beta =1/3.5d
    // logKi67_det used to compare to structured PDE models where Ki67 is lost deterministically
    // Ki67_stoch used to compare to ODE models with Ki67 hi/lo compartments (prob beta*dt of Ki67hi to Ki67lo in interval dt)
    int ndivs; // number of divisions since leaving thymus
    float last_division_time; // time at which this cell last divided
    float export_time;// time at which its ancestor left the thymus

public:
    cell(); // default constructor fn.
    cell(int p1, float p2, float p3, bool p4, int p5, float p6, float p7); // general constructor fn.
    void set_cloneID(int x){cloneID=x;}
    void set_logGFP(float x){logGFP=x;}
    void set_logKi67_det(float x){logKi67_det=x;}
    void set_Ki67_stoch(bool x){Ki67_stoch=x;}
    ...
    float get_logGFP() {return(logGFP);}
    float get_logKi67_det() {return(logKi67_det);}
    bool get_Ki67_stoch(){return (Ki67_stoch);}
...
cell::cell(){ //default constructor
    cloneID=0;
    logGFP=0.;// absolute expression =1
    logKi67_det=0.;
    Ki67_stoch=false;
    ndivs=0;
...
cell::cell(int p1, float p2, float p3, bool p4, int p5, float p6, float p7){ //general constructor
    cloneID=p1;
    logGFP=p2;
    logKi67_det=p3;
    Ki67_stoch=p4;
...
float sp_numbers(float t, float params[]){ // mature SP thymocyte numbers - spline defs
    ...
    int CELLTYPE = (int)params[0];
    float  dpt0, T0=params[2], sp_numbers, theta0, theta_f,  n, X,  q ;

    // define the splines for SP4 and SP8
    if (CELLTYPE==4) { theta0  = 4.3E5;  theta_f = 1.8E3;   n = 2.1;    X = 30.0;  q = 3.7;}
    if (CELLTYPE==8) { theta0  = 9E4;    theta_f = 68.0;    n = 3.0;    X = 25.0;  q = 4.25; }
    dpt0 = t - T0;     // days post t0
    if (dpt0<0.) dpt0=0.;
    sp_numbers = theta0 + (theta_f * pow(dpt0,n)) * (1 - (pow(dpt0,q)/(pow(X,q) + pow(dpt0,q))));
    return sp_numbers;
}

float sp_ki67(float t, float params[]){ // Ki67+ fraction in mature SP4 and SP8 thymocytes
    float eps_0, eps_f, A, sp_ki67;
    int CELLTYPE = (int) params[0];
    if(t<5.) t=5.; //We don't know what it looks like < 5d, so extrapolate
    //parameters estimated from spline fit to the timecourse of ki67 proportions of SP4/SP8. d5 onwards
    if (CELLTYPE==4) { eps_0 = 0.14965320;  eps_f = 0.03470231;  A = 3.43078629;}
    if (CELLTYPE==8) { eps_0 = 0.24510453;  eps_f = 0.01559996;  A = 14.83715328;}
    sp_ki67 = exp(-eps_f * (t + A)) + eps_0;
    return sp_ki67;
}
            ...
            int *spacelistlength, float current_time, float params[], float THYMIC_EXPORT_RATE_CONSTANT)
{
    ...
    float matureSP_Ki67pos_frac, runif, age, logKi67_det, logGFP, last_division_time, time_since_last_division, export_time, p_div, p_loss;
    cell *this_cell;
    ...
    // call these with LOG_GFPdist_Ki67HI(/LO)_RTE(generator)
    ...
    for(i=0;i<*poolsize;i++){ // loop through all cells alive at the start of this timestep
        ...
        logGFP  =logGFP - TSTEP*GFP_LOSS_RATE; // GFP drops with time
        (*this_cell).set_logGFP(logGFP);
